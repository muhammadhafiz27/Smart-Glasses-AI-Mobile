...
// Sumber:
//  • Sheppard & Wolffsohn (2018) – Digital Eye Strain (TFOS DEWS II)
//  • Holden et al. (2016) – Global Myopia Prevalence (Ophthalmology)
//  • Wen et al. (2022) – Blue Light Glasses & Visual Fatigue (Ophthalmology)
//  • AAO – Recommended UV protection guidelines (2023)
//  • Ciuffreda et al. – 20-20-20 Rule for accommodative spasm reduction
// ─────────────────────────────────────────────
class _RecommendationEngine {
  static List<_Recommendation> evaluate(LifestyleController ctrl) {
    final List<_Recommendation> results = [];
    final activity = ctrl.primaryActivity.toLowerCase();
    final screenTime = ctrl.screenTimeHours;
    final isOutdoor = ctrl.outdoorActivity;

    // RULE 1 – High screen time (≥6 jam/hari)
    // Dasar: Sheppard & Wolffsohn (2018) melaporkan 65% pengguna layar ≥6 jam
    // mengalami Digital Eye Strain (DES). Blue-light lens mengurangi gejala
    // pada kondisi photopic (Wen et al., 2022).
    if (screenTime >= 6) {
      results.add(const _Recommendation(
    ...
    // RULE 2 – Aktivitas kantor / membaca
    // Dasar: Anti-reflective coating mengurangi back-glare dari layar LCD/LED
    // hingga 99%, meningkatkan kontras visual (AAO Clinical Practice, 2023).
    if (activity.contains('office') ||
    ...
    // RULE 3 – Aktivitas outdoor / mengemudi
    // Dasar: UV-A & UV-B kronis memicu katarak (WHO, 2022). Polarized lens
    // mengurangi disability glare saat berkendara hingga 30% (Rosenfield, 2016).
    if (isOutdoor ||
    ...
    // RULE 4 – Aktivitas malam / screen time tinggi
    // Dasar: Paparan cahaya malam menghambat sekresi melatonin dan memperburuk
    // kualitas tidur (Chang et al., 2015, PNAS). Night-mode + AR lens direkomendasikan.
    if (activity.contains('night') || screenTime >= 8) {
      results.add(const _Recommendation(
    ...
    // RULE 5 – 20-20-20 Rule (selalu ditampilkan jika ada screen time)
    // Dasar: Ciuffreda et al. membuktikan istirahat singkat setiap 20 menit
    // mengurangi accommodative spasm dan gejala DES secara signifikan.
    if (screenTime > 0) {
      results.add(const _Recommendation(
    ...
    // RULE 6 – Myopia prevention untuk screen time sangat tinggi
    // Dasar: Holden et al. (2016) memproyeksikan 50% populasi dunia miopi pada
    // 2050. Outdoor time ≥2 jam/hari mengurangi onset myopia pada anak & remaja.
    if (screenTime >= 6 && !isOutdoor) {
      results.add(const _Recommendation(
    ...
    // Jika tidak ada rule yang cocok, tampilkan rekomendasi umum
    if (results.isEmpty) {
      results.add(const _Recommendation(
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: [
                            BoxShadow(
                              color: const Color(0xFF4F6DFF).withOpacity(0.35),
                              blurRadius: 20,
      ...
      child: Row(
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(
                colors: [rec.gradientStart, rec.gradientEnd],
              ),
              borderRadius: BorderRadius.circular(16),
            ),

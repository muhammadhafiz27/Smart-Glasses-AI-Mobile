  ...
  // Tambah kata kunci setiap ada frame baru
  class GlassesAssetMapper {
    static const Map<String, String> _map = {
      // Angular / Rectangular → Wayfarer
    ...
    static String fromName(String frameName) {
      ...
      // Cek exact phrase dulu (untuk multi-kata seperti "half-rimless")
  ...
  class _TryOnScreenState extends State<TryOnScreen>
    ...
    Future<void> _processCameraImage(CameraImage image) async {
      try {
        ...
        if (mounted) {
          setState(() => _detectedFace = faces.isNotEmpty ? faces.first : null);
        }
      } catch (e) {
    ...
    Widget build(BuildContext context) {
      ...
      // ✅ Ambil nama frame yang sedang dipilih
      final selectedFrameName = (result != null && result.frames.isNotEmpty)
      ...
      return Scaffold(
        ...
        body: Stack(
          ...
          children: [
            ...
            // ── GLASSES OVERLAY ──
            // ✅ Kirim selectedFrameName ke builder
            if (_detectedFace != null && _isCameraReady)
              ..._buildGlassesPositioned(screenSize, selectedFrameName),
                  ...
                  children: [
                    ...
                    if (result != null && result.frames.isNotEmpty)
                      _FrameSelector(
                        ...
                        onSelect: (i) =>
                            setState(() => _selectedFrameIndex = i),
                      )
                    else
                      const Text(
                        'No frames loaded.',
    ...
    List<Widget> _buildGlassesPositioned(
      Size screenSize,
      String frameName,
    ) {
      ...
      if (leftEye != null && rightEye != null) {
        ...
        // Hitung sudut kemiringan antara dua mata
        ...
        rotationAngle = -math.atan2(dy, dx); // negatif karena mirror kamera depan
      } else {
        rawEyeX = face.boundingBox.center.dx;
        rawEyeY = face.boundingBox.top + face.boundingBox.height * 0.35;
        // Gunakan headEulerAngleZ sebagai fallback
        rotationAngle = ((face.headEulerAngleZ ?? 0) * math.pi / 180);
      }

      // Rotasi 270° → dimensi tertukar
      final double imgW = _imageSize.height;
      final double imgH = _imageSize.width;

      final double scaleX = screenSize.width  / imgW;
      final double scaleY = screenSize.height / imgH;

      // Mirror horizontal untuk kamera depan
      final double screenX = screenSize.width - (rawEyeX * scaleX);
      final double screenY = rawEyeY * scaleY;

      final double faceScreenW = face.boundingBox.width * scaleX;
      final double glassesWidth = (faceScreenW * 1.6).clamp(80.0, 320.0);
      ...
      return [
        Positioned(
          ...
          child: Transform.rotate(
            angle: rotationAngle, // ← rotasi mengikuti kemiringan kepala
            child: ScaleTransition(
              scale: _pulseAnim,
              child: _GlassesWidget(
                width: glassesWidth,
                assetPath: assetPath,
              ),
            ),
